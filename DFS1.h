...
public:
    ...
    void DfsUtil(vector<State<T> *> visited, State<T> *curState) {
        ...
        // ending condition to the recursion
        if ((*curState).Equals(this->getSearchable1()->getGoalState())) {
            return this->backTrace(curState);
        }
        this->openPriority_queue.pop();
        // entering the successors to the priority list too
        list<State<T> *> successors = this->getSearchable1()->createSuccessors(curState);
        for (State<T> *n: successors) {
            auto iter = find(visited.begin(), visited.end(), n);
            // if we didn't visit in this state before
            if (iter == visited.end()) {
